...
gradient <- function(x, y, z, main = NULL, ..., FUN = function(x,
  ...
  if (!add) {
    ...
    while (gap > tol & i <= nmax) {
      ...
      arrows(xy[1:i, 1], xy[1:i, 2], newxy[1:i, 1], newxy[
        1:i,
        2
...
lc <- structure(list(x = c(
  -0.798325262308313, -0.801452784503632,
  ...
  0.579288125191777, 0.694047253758822,
  ...
  0.673181957655723, 0.639275851488187
)), .Names = c("x", "y"))

f2 <- function(x, y) sin(1 / 2 * x^2 - 1 / 4 * y^2 + 3) *
    cos(2 * x + 1 - exp(y))
par(
  mar = c(2, 2, 2, 1), cex.axis = 1, cex.lab = 1,
  tcl = -0.5, mgp = c(2, 1, 0)
)
x <- seq(-1, 1, length = 40)
...
tmp0 <- tmp1 <- NULL
for (i in 1:20) {
  tmp <- gradient(x, y, z, axes = FALSE, FUN = f2, rg = c(
    -0.8,
    -0.8, 0.7, 2
  ), init = c(lc$x[i], lc$y[i]), gamma = runif(
    1,
    0.05, 0.15
  ), tol = 1e-04, nmax = ceiling(runif(
    1, 5,
    13
  )), add = TRUE)
